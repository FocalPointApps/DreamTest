...
class _VoiceCallScreenState extends State<VoiceCallScreen> {
  ...
  Future<void> updateCallCost() async {
    try{
    if(callId!=""&&widget.appointment!=null){
      print("close111122211111111");
      print(widget.appointment?.appointmentId);
      final response = await http.post(
          Uri.parse('https://us-central1-dream-43bb8.cloudfunctions.net/updateCallCost'),
          body: {
            'callid': callId,
            'appointmentId':widget.appointment?.appointmentId
          });
      print("close111122211111111 fffffff");
      print(response.body);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        // Color(0xFF9D3A82),//Colors.lightBlueAccent,//Theme.of(context).accentColor,
        body: Container(
          child: SafeArea(
            child: Padding(
...
/*  firstTime? Container(color: Colors.red,width: size.width,child: Padding(
  ...
  Future<void> callDone() async {
    var answeredCallNum = 0, packageCallNum = 0;
    try {
      setState(() {
        endingCall = true;
      });
